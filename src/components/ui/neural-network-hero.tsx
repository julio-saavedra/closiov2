...
const fragmentShader = `
  ...
  void main() {
    ...
    // Navy blue color palette - increased brightness
    vec3 navyBlue = vec3(0.172, 0.4, 1.0);
    vec3 darkNavy = vec3(0.03, 0.06, 0.14);
    vec3 midNavy = vec3(0.1, 0.22, 0.5);

    // Use the CPPN output as intensity to blend between dark and bright navy
    float intensity = (cppnColor.r + cppnColor.g + cppnColor.b) / 3.0;

    // Create color variation based on CPPN output - increased visibility
    vec3 finalColor = mix(darkNavy, midNavy, intensity);
    finalColor = mix(finalColor, navyBlue, pow(intensity, 1.8) * 0.55);
...
export default function NeuralNetworkHero({
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden bg-[#030712]">
      ...
      <div
        className={`relative z-10 mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16 transition-all duration-700 ${
          isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
        }`}
      >
        <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 backdrop-blur-sm">
          <span className="text-[10px] font-light uppercase tracking-[0.08em] text-white/70">{badgeLabel}</span>
          <span className="h-1 w-1 rounded-full bg-[#2C66FF]" />
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2">
          {ctaButtons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className={`rounded-2xl border px-5 py-3 text-sm font-light tracking-tight transition-all focus:outline-none focus:ring-2 focus:ring-[#2C66FF]/50 duration-300 ${
                button.primary
                  ? "border-[#2C66FF]/30 bg-[#2C66FF] text-white hover:bg-[#2B4FB3]"
        ...
        <ul className="mt-8 flex flex-wrap gap-6 text-xs font-extralight tracking-tight text-white/60">
          {microDetails.map((detail, index) => (
            <li key={index} className="flex items-center gap-2">
              <span className="h-1 w-1 rounded-full bg-[#2C66FF]" /> {detail}
            </li>
          ))}
        </ul>
      </div>

      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-black to-transparent" style={{ zIndex: 5 }} />
    </section>
  );
}
...
declare module '@react-three/fiber' {
  interface ThreeElements {
    cPPNShaderMaterial: React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement> & {
      ref?: React.Ref<ShaderMaterialRef>;
      side?: THREE.Side;
    };
  }
}
